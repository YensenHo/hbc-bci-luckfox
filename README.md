...
> **一块 ¥338 的国产芯片，完成脑电解码 + 人体通信回传的融合闭环。**
> EEGNet 端侧推理 **18.5ms**（与 PyTorch 误差 8e-07），EQS-HBC 人体信道作为回传链路——不用 RF，用人体本身传神经信号。
...
| 端侧 CPU 推理 | **18.5 ms** | 与 PyTorch 误差 8e-07（NPU 1.2ms 速度真实，但 INT8 精度已证伪，见 RESULTS.md）|
| EQS-HBC 体表信道 | **46.3 dB @ 1MHz** | IT'IS V4.2 4-Cole-Cole，与 Sen 组 BP-QBC 实测吻合 |
...
脑机接口（BCI）的无线化是必然趋势，但主流方案（RF 射频）功耗高、还要天线对准。**EQS-HBC（电准静态人体通信）用人体本身作为信道**，功耗比 RF 低几个数量级。

这个项目把这条链路做到了**一块 ¥338 的国产芯片**上：

```
头皮电极 → ADS1299(采集) → RV1106 CPU(EEGNet 解码) → AD9833(HBC 1MHz 载波) → 人体信道 → 接收
```
...
### 2. 端侧推理（CPU 版，18.5ms）
- EEGNet 纯 C 前向，单次推理 18.5ms（对 2s 窗口有 100× 余量）
- NPU 加速（1.2ms / 15×）**速度真实，但 rknn-toolkit2 对 EEGNet 的 INT8 量化精度不达标**（全测试集 argmax 一致率仅 42-58%，已证伪），故解码正确性以 CPU 版为准（见 RESULTS.md §5）
...
- **信道是模型，不是测量**：46.3dB 来自参数化信道模型（IT'IS + 返回路径电容），**不代表穿过真实人体的实测传输**。
