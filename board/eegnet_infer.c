 ...
 *
 * 结构：
 *   conv1(1→8, 1×64, pad L31/R32) → depthwise(8→16, 8×1, groups8)
 *   → ELU → AvgPool(1,4) → sep_depthwise(16→16, 1×16, pad L7/R8)
...
int eegnet_infer(const float *x, float *logits) {
    /* ---- Block 1 ---- */
    /* conv1: 1→F1, kernel (1,64), pad 左31 右32 */
        ...
        for (int c = 0; c < NCH; c++) {
            for (int t = 0; t < T; t++) {
                ...
                for (int k = 0; k < 64; k++) {
                    ...
                    if (idx >= 0 && idx < T) {
                        s += conv1_w[(f * 1 * 1 + 0) * 64 + k] * x[c * T + idx];
                    }
                }
                c1[f][c][t] = s;
            }
    ...
    /* ---- Block 2 ---- */
    /* sep_depthwise: F2→F2, kernel (1,16), pad 左7 右8 */
    ...
    for (int c = 0; c < F2; c++) {
        for (int t = 0; t < T4; t++) {
            ...
            for (int k = 0; k < 16; k++) {
                ...
                if (idx >= 0 && idx < T4) {
                    s += sep_dw_w[(c * 1 * 1 + 0) * 16 + k] * p1[c][idx];
                }
            }
            sdw[c][t] = s;
        }
